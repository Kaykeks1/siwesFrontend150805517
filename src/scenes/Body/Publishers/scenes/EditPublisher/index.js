...
class EditPublisher extends React.Component {
    constructor(props) {
      super(props);
      this.state = {
        modalIsOpen: false,
    ...
    editPublisher(e){
      ...
      axios.patch(`http://localhost:1337/publisher/${self.props.data.id}`,{
        firstName: firstName,
        lastName: lastName,
    ...
    render() {
      ...
      console.log("Here", this.props.data.book.map(item => ({value: item.id, label: `${item.title} (${item.author})`})))
    return (
      <div>
        ...
        >
          ...
          <div className="publisher-form-header"><h2>Edit Publisher Form</h2></div>
